...
# irisan target imputasi dengan stpim20
stpim20_imptpt <- stpim20 |>
  filter(Kipb %in% sisa_target21$kip)

# sisa target imputasi 
sisa_target20 <- sisa_target21 |>
  filter(!(kip %in% stpim20_imptpt$Kipb))

```

Dari target imputasi TPT sebanyak **`r nrow(target_imptpt)`**, ditemukan irisan dengan data STPIM tahun-tahun sebelumnya sebagai sumber imputasi, a.l. sebanyak **`r nrow(stpim22_imptpt)` KIP dari STPIM22, `r nrow(stpim21_imptpt)` dari STPIM21, `r nrow(stpim20_imptpt)` dari STPIM20**, sisanya `r nrow(sisa_target20)` KIP tidak ditemukan di STPIM20-22.

**`r nrow(sisa_target20)` KIP tanpa historis data STPIM akan diimputasi dengan tenaga kerja = 20** dengan pertimbangan bahwa skala usaha berdasarkan variabel tenaga kerja, investasi, dan omset di DPA23nya masing-masing adalah skala usaha mikro dan kecil.

```{r }

#kelompokkan basis data historis 2020-2022 >> mapping variabel apa yang diperlukan: kipb, dprovi, dkabup, disic5, rincian tk, rincian output, rincian input.
