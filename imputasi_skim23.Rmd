...
Terlihat sebaran data masih ada outlier di mana input sangat kecil, tetapi output sangat tinggi. Setelah dicek, KIP 331411444 merupakan perusahaan maklun di mana nilai input berasal dari listrik dan lainnya, tanpa nilai bahan baku.
...
#knn_fit <- train(Output21 ~ . ,
#                 data = train_data,
#                 method = "knn",
#                 preProc = c("center", "scale"), #standardize
#                 tuneGrid		= data.frame(.k = seq(from = 1, to = (ncol(train_data) - 1), by = 5)),  # Decide k parameter!
...
#rmse(actual = validation_data$Output21, predicted = knn_preds)



# Training RF
#set.seed(777)
#rf_fit <- train(Output21 ~ . ,
#                data = train_data,
#                method = "rf",
#                tuneGrid		= data.frame(mtry = seq(from = 1, to = (ncol(train_data) - 1), by = 10)),
...
#rmse(actual = validation_data$Output21, predicted = rf_preds)
