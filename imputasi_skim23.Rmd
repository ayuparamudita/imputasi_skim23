...
- Menghitung pertumbuhan biaya input 2022 terhadap 2021 per provinsi per 2 digit KBLI. Variasi pertumbuhan biaya input pada KBLI 13 maupun 14 sangat heterogen antarprovinsi seperti yang ditunjukkan oleh Grafik.
...
sum_inp22 <- stpim22_tpt |>
  group_by(Dprovi22, kbli2_stpim) |>
  summarise(total_inp22 = sum(Iinput22, na.rm = TRUE), .groups = "drop") |>
  mutate(prov_kbli2 = paste(Dprovi22, kbli2_stpim, sep = ""))

sum_inp21 <- stpim21_tpt |>
  group_by(Dprovi21, kbli2_stpim) |>
  summarise(total_inp21 = sum(Iinput21, na.rm = TRUE), .groups = "drop") |>
  mutate(prov_kbli2 = paste(Dprovi21, kbli2_stpim, sep = ""))


#Menghitung pertumbuhan biaya input per provinsi dan kbli2
sum_inp21 <- sum_inp21 |>
  left_join(sum_inp22[,c("prov_kbli2", "total_inp22")], by = "prov_kbli2") |>
  mutate(
    grwt_inp = ifelse(
...
barplot_inp13 <- barplot(
  sum_inp21[sum_inp21$kbli2_stpim == 13, ]$grwt_inp,  # y-axis data: grwt_inp for kbli2_stpim == 13
  names.arg = sum_inp21[sum_inp21$kbli2_stpim == 13, ]$Dprovi21,  # x-axis labels: prov_kbli2 for kbli2_stpim == 13
  main = "Pertumbuhan Input per Provinsi KBLI 13 (%)",
  xlab = "Provinsi",
  col = "maroon",
  axes = FALSE
)
...
text(
  x = barplot_inp13,  # x positions of the bars
  y = sum_inp21[sum_inp21$kbli2_stpim == 13, ]$grwt_inp,  # y positions (the heights of the bars)
  labels = round(sum_inp21[sum_inp21$kbli2_stpim == 13, ]$grwt_inp, 0), 
  pos = 3,  # Position of the text (3 means above the bars)
  col = "black",  # Text color
  cex = 0.8  # Text size (adjust as needed)
)

#Barplot pertumbuhan input KBLI 14
barplot_inp14 <- barplot(
  sum_inp21[sum_inp21$kbli2_stpim == 14, ]$grwt_inp,  # y-axis data: grwt_inp for kbli2_stpim == 13
  names.arg = sum_inp21[sum_inp21$kbli2_stpim == 14, ]$Dprovi21,  # x-axis labels: prov_kbli2 for kbli2_stpim == 13
  main = "Pertumbuhan Input per Provinsi KBLI 14 (%)",
  xlab = "Provinsi",
  col = "maroon",
  axes = FALSE
)
...
text(
  x = barplot_inp14,  # x positions of the bars
  y = sum_inp21[sum_inp21$kbli2_stpim == 14, ]$grwt_inp,  # y positions (the heights of the bars)
  labels = round(sum_inp21[sum_inp21$kbli2_stpim == 14, ]$grwt_inp, 0), 
  pos = 3,  # Position of the text (3 means above the bars)
  col = "black",  # Text color
  cex = 0.8  # Text size (adjust as needed)
)
...
- Setelah didapatkan data histori pertumbuhan input 2022 terhadap 2021, match-kan dengan provinsi dan KBLI target imputasi 2023. Jika ada yang tidak memiliki histori pertumbuhan input, maka pendekatan dilakukan dengan melakukan imputasi pertumbuhan dengan median pertumbuhan nasional.

```{r}
#Match pertumbuhan per provinsi dan KBLI dari data historis dengan target imputasi 2023
tab_stpim_imptpt <- stpim_imptpt |>
  group_by(prov, kbli2_stpim) |>
  summarise(total_nimp = n(), .groups = "drop") |>
  mutate(prov_kbli2 = paste(prov, kbli2_stpim, sep = "")) |>
  left_join(sum_inp21[, c("prov_kbli2", "grwt_inp")], by = "prov_kbli2") |>
  mutate(grwt_inp_imp = ifelse(is.na(grwt_inp), median(grwt_inp, na.rm = TRUE), grwt_inp))
  
```

```{r}
#- Melakukan imputasi komposisi biaya input dengan adjustment pertumbuhan biaya input 22-21 per kbli per prov?
