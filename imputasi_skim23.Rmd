...
- Menghitung pertumbuhan biaya input 2022 terhadap 2021 per provinsi per 2 digit KBLI. Variasi pertumbuhan biaya input pada KBLI 13 maupun 14 sangat heterogen antarprovinsi seperti yang ditunjukkan oleh Grafik di bawah.
...
ggplot(sum_inp21_13, aes(x = reorder(Dprovi21, grwt_inp), y = grwt_inp)) +
  ...
  geom_text(aes(label = round(grwt_inp, 2)), # Add labels rounded to 2 decimal places
            hjust = -0.1, # Adjust position to the left of the bars
            size = 3.5,
            colour = "gray") + # Adjust text size
...
- Jika ada yang tidak memiliki histori pertumbuhan input, maka pendekatan dilakukan dengan melakukan imputasi pertumbuhan dengan median pertumbuhan nasional.
- Jika pertumbuhan biaya input terlalu ekstrem (kurang dari -40 persen s.d. lebih dari 400 persen), lakukan penyesuaian dengan men-setting range menjadi [-40 persen, 400 persen] (Provinsi Jateng pertumbuhannya -38, sehingga ditetapkan -40 agar mempertahankan fenomena TPT di provinsi dominan TPT).
...
tab_stpim_imptpt <- stpim_imptpt |>
  ...
  # Adjustment for missing history data
...
stpim_imptpt_trial <- stpim_imptpt |> 
  ...
  left_join(
    ...
    by = "kip") |>
  mutate(prov_kbli2 = paste(prov, kbli2_stpim, sep = "")) |>
  left_join(tab_stpim_imptpt[, c("prov_kbli2", "grwt_inp_imp")], by = "prov_kbli2") |>
...
```{r}
#- imputasi no histori: mean tk = 20-99 per kbli2 per prov
#- growth dipecah lagi dengan tk B(99+) dan S(20-99)
...
## tk : historis
## pengeluaran pekerja : historis w/ adjustment
## total output : rasio/ml?
## breakdown output 
## biaya input : historis w/ adjustment growth input by kbli2 & prov
